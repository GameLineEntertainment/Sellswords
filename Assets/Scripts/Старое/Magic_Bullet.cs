...
{
    [Header("Ничего и никогда тут блядь не трогать!")]
  ...
  //  public float LifeTime = 5;
    ...
    {          
      //  print(pr.name);
        ...
        Target = pr.Target;
      //  base.ProgectileStart();

        Settings.Damage = 1;
        gameObject.tag = pr.tag;
    ...
    {
        RaycastHit hit;
        if (!isCollided && Physics.Raycast(transform.position, transform.forward, out hit, 10, CollidesWith))
        {
    ...
    void UpdateWorldPosition()
    {
        ...
        {
            currentSpeed = Mathf.Clamp(currentSpeed - Settings.Speed * Dampeen * Time.deltaTime, MinSpeed, Settings.Speed);
    ...
    void OnCollisionBehaviour(RaycastHit hit)
    {
        ...
        foreach (var effect in EffectsOnCollision)
        {
            ...
            Destroy(instance, Settings.LifeTime);
        }

        
        if (Killed == false)
        {
            MiniEnemyAI go = hit.collider.gameObject.GetComponent<MiniEnemyAI>();
            go.AntiGravity = true;
            go.Health(Settings.Damage, gameObject.tag);
