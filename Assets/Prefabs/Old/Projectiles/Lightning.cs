    ...
    private void UpdateRaycast()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out raycastHit, RaycastDistance))
        {
            ...
            if (CollidedInstances.Count == 0)
            {
                ...
                // Моё
                Rigidbody RG = raycastHit.collider.gameObject.GetComponent<Rigidbody>();
                if (RG != null)
                {
                    Vector3 ForceDir = raycastHit.point - PR.transform.position;
                    RG.AddForce(ForceDir, ForceMode.Impulse);
                }
